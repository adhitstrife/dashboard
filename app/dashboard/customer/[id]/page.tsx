"use client"
import { ActionIcon, AppShell, Badge, Box, Button, Card, Center, Flex, Grid, Group, Image, Modal, NumberInput, Progress, Select, Table, Tabs, Text, Title, Tooltip, useMantineTheme } from "@mantine/core";
import DashboardLayout from "../../layout";
import useUserProfile from "@/hooks/auth/useUserProfile";
import { use, useEffect, useState } from "react";
import { IconCalendarCheck, IconClockCheck, IconEdit, IconEye, IconFileDollar, IconLogs, IconMap, IconRowRemove, IconTable, IconUser, IconUserDollar, IconUserX } from "@tabler/icons-react";
...
export default function customerDetail({ params }: { params: Promise<{ id: number }> }) {
    ...
    const { customerDetail, getDetailCustomer, isLoadingGetDetailCustomer, setCustomerDetail } = useGetCustomerDetail()
                ...
                <div className="header-page">
                    <Flex justify={'space-between'}>
                        <Box>
                            ...
                            <Text size='sm' mt={10} style={{ color: theme.colors['secondary-gray'][9] }}>Here is the detail information of customer</Text>
                        </Box>
                        <Box>
                ...
                <Card withBorder radius={"md"} px={20} py={30} mah={'screen'} mt={20}>
                    {customerDetail && (
                        <Box>
                            ...
                            <Title mt={30} order={3} ta={"center"}>{customerDetail.results.name}</Title>
                            <Box mt={40}>
                                <Tabs color="primary-red" variant="outline" defaultValue="detail">
                                    ...
                                    <Tabs.Panel value="detail">
                                        <Grid mt={40}>
                                            ...
                                            <Grid.Col span={6}>
                                                <DataLabel label="Address" value={customerDetail.results.address ? customerDetail.results.address : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="Contact Person" value={customerDetail.results.contact_person ? customerDetail.results.contact_person : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="Phone Number" value={customerDetail.results.phone ? customerDetail.results.contact_person : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="Province" value={customerDetail.results.province ? customerDetail.results.province.name : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="City" value={customerDetail.results.city ? customerDetail.results.city.name : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="District" value={customerDetail.results.district ? customerDetail.results.district.name : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="Sub District" value={customerDetail.results.sub_district ? customerDetail.results.sub_district.name : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="NPWP" value={customerDetail.results.npwp ? customerDetail.results.npwp : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="Permission Letter" value={customerDetail.results.permission_letter ? customerDetail.results.permission_letter : "-"} />
                                            </Grid.Col>
                                            <Grid.Col span={6}>
                                                <DataLabel label="Status" withValue={false} value="" />
                                                {customerDetail.results.status == 'In Review' ? (
                                                    <Button color="primary-red" onClick={handleOpenApproveModal}>
                                                        Aprrove Customer
                                                    </Button>
                                                ) : (
                                                    <Badge color="primary-red" size="lg">Approved</Badge>
                                                )}
                                            </Grid.Col>
                                        </Grid>
                                    </Tabs.Panel>
                                    <Tabs.Panel value="sales">
                                        <Button color="primary-red" mt={20} variant="filled" onClick={() => handleAssignSales(customerDetail.results.id)}>
                                            Assign Sales
                                        </Button>
                                                        ...
                                                        <Table.Tr key={index}>
                                                            ...
                                                            <Table.Td>
                                                                <Tooltip label="Unassign This Sales">
                                                                    <ActionIcon onClick={() => handleUnassignSales(sales.id)} variant="transparent">
